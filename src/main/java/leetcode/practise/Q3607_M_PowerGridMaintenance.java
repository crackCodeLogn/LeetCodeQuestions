  ...
  private void bfs(int src, int colorSeq) {
    ...
    while (!queue.isEmpty()) {
      ...
      if (!curNode.isColored()) {
        ...
        if (!colorStationsMap.containsKey(colorSeq))
          colorStationsMap.put(colorSeq, new TreeSet<>());
        colorStationsMap.get(colorSeq).add(cur);
        // colorStationsMap.compute(colorSeq, (k,v) -> v == null ? new TreeSet<>() : v).add(cur); //
        // these bumped the 442 ms to 477 ms

        if (!colorOperationalMap.containsKey(colorSeq))
          colorOperationalMap.put(colorSeq, new TreeSet<>());
